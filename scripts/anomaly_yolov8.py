...
th_iou = args.th_iou
th_sec = args.th_sec
data_name = f"anomaly_yolo_sec{th_sec}_iou{th_iou}"
data_root = "datasets"
...
def create_dataset(imgs, idxs, data_root, stage):
...
if args.create_dataset:
    ...
    create_dataset(imgs, train_idxs, data_root, "train")
    create_dataset(imgs, test_idxs, data_root, "test")
...
v_num = args.version
yolo_result_dir = f"runs/{data_name}"
if args.train:
    # train YOLO
    model = YOLO("yolov8n-cls.pt")
    model.train(data=f"{data_name}", epochs=100, task="classify")
    ...
    if os.path.exists(yolo_result_dir):
        ...
        yolo_result_dir = f"runs/{data_name}/v{v_num}"
...
train_paths = sorted(
    glob(os.path.join(f"datasets/{data_name}", "train", "**", "*.jpg"))
)
test_paths = sorted(glob(os.path.join(f"datasets/{data_name}", "test", "**", "*.jpg")))
